                  ...
                  <Link href={`/blog/${post.slug}`}
                    ...
                    {post.cover_image ? (
                      <div className="h-44 bg-[#F8FAFC] overflow-hidden">
                        ...
                        <img src={post.cover_image.includes('supabase.co') ? post.cover_image.replace('/storage/v1/object/public/', '/storage/v1/render/image/public/') + '?width=600&quality=70&resize=cover' : post.cover_image} alt={post.title} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-300" />
                      </div>
                    ) : (
